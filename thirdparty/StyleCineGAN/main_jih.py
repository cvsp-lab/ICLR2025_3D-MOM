...
from thirdparty.StyleCineGAN.option import Options
from thirdparty.StyleCineGAN.utils.model_utils import load_encoder, load_stylegan2
from thirdparty.StyleCineGAN.utils.ip_utils import read_image, resize_tensor, to_numpy, to_tensor, make_sg2_features, make_sg2_features_fs, read_image_jih
from thirdparty.StyleCineGAN.utils.utils import clip_img, gan_inversion, predict_mask
# from thirdparty.StyleCineGAN.utils.flow_utils import z_score_filtering, flow2img
from thirdparty.StyleCineGAN.utils.cinemagraph_utils import feature_inpaint, resize_flow, resize_feature


# flow, #image
def VideoGenerator(train_data,center_idx=2):

    print(f'center view at {center_idx}')
    frame_2 = train_data["frames"][center_idx]

    image = frame_2["image"]
    image = image.resize((1024, 1024))

    our_flow = frame_2["our_flow"][0]
    our_flow = F.interpolate(our_flow, size=(1024, 1024), mode='bilinear', align_corners=False)
    ...
    # read images ------------------------------------------------------------------------------------
    # basename_input = (opts.input_dir).split("/")[-1]
    # torch_input = read_image(f"{opts.img_path}/{basename_input}.png", dim=1024, is_square=True, return_tensor=False)
    torch_input = read_image_jih(image, dim=1024, is_square=True, return_tensor=False)
    ...
    if opts.vis == "True":
        ...
        # mp.write_image(f"{opts.input_dir}/recon.png", img_list)
    
    
    # load flow ----------------------------------------------------------------------------------
    print(f"\n>>> Loading Flow...")
    # flow = np.load(f"{opts.img_path}/{basename_input}_motion.npy")
    # flow = torch.from_numpy(our_flow).to(device)
    flow = our_flow.to(device)
    # flow *= 3
    print(">>> Done -------------------------- \n")
