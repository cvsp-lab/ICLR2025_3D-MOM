...
class Inpainter():
    ...
    def sequential_inpainting(self, rgb, depth, depth_bins):
        ...
        for i in range(num_bins):
            alpha_i = (depth >= depth_bins[i]) * (depth < depth_bins[i + 1])        # depth bins 클러스터링 기준 값으로 RGB 레이어 분리
            ...
            if i == 0:
                ...
                # inpaint rgb
                inpainted_rgba_i = self.inpaint_rgb(holes, context, context_rgb, edge)      #hole 근처 40 픽셀씩 채워지도록 box를 만듬
