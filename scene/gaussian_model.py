...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int, args):
        ...
        self._deformation = deform_network(args)    # deform argment 전달
        ...
        self._scene_flow = torch.empty(0)
        self.iter_lr = 1.0
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float, time_line: int, TrainData_path: str, flow_scale: int):
        ...
        """ test """
        scene_flow = torch.load(os.path.join(os.path.dirname(TrainData_path), 'scene_flow.pth')).T.float().cuda()
        scene_flow*=flow_scale
        print("flow_scale: ",flow_scale)
        
        self._scene_flow = scene_flow.detach().requires_grad_(False)
    ...
    def training_setup_jih(self, training_args, stage):
        self.percent_dense = training_args.percent_dense
        self.xyz_gradient_accum = torch.zeros((self.get_xyz.shape[0], 1), device="cuda")
        self.denom = torch.zeros((self.get_xyz.shape[0], 1), device="cuda")
        self._deformation_accum = torch.zeros((self.get_xyz.shape[0],3),device="cuda")
        ...
        if stage == 'fine':
            self.iter_lr = 0.0
        print("stage ",stage)
        print(f"iter_lr {self.iter_lr}")

        l = [
            {'params': [self._xyz], 'lr': training_args.position_lr_init * self.spatial_lr_scale, "name": "xyz"},
            {'params': list(self._deformation.get_mlp_parameters()), 'lr': training_args.deformation_lr_init * self.spatial_lr_scale, "name": "deformation"},
            {'params': list(self._deformation.get_grid_parameters()), 'lr': training_args.grid_lr_init * self.spatial_lr_scale, "name": "grid"},
            {'params': [self._features_dc], 'lr': training_args.feature_lr * self.iter_lr, "name": "f_dc"},
            {'params': [self._features_rest], 'lr': training_args.feature_lr / 20.0 * self.iter_lr, "name": "f_rest"},
            {'params': [self._opacity], 'lr': training_args.opacity_lr * self.iter_lr, "name": "opacity"},
            {'params': [self._scaling], 'lr': training_args.scaling_lr * self.iter_lr, "name": "scaling"},
            {'params': [self._rotation], 'lr': training_args.rotation_lr * self.iter_lr, "name": "rotation"}
        ...
        self.xyz_scheduler_args = get_expon_lr_func(lr_init=training_args.position_lr_init*self.spatial_lr_scale,
                                                    ...
                                                    lr_delay_mult=training_args.position_lr_delay_mult,
                                                    max_steps=training_args.position_lr_max_steps)
        # self.xyz_scheduler_args = get_expon_lr_func(lr_init=training_args.position_lr_init*self.spatial_lr_scale*self.iter_lr,
        #                                             lr_final=training_args.position_lr_final*self.spatial_lr_scale*self.iter_lr,
        #                                             lr_delay_mult=training_args.position_lr_delay_mult,
        #                                             max_steps=training_args.position_lr_max_steps)
        self.deformation_scheduler_args = get_expon_lr_func(lr_init=training_args.deformation_lr_init*self.spatial_lr_scale,
                                                    lr_final=training_args.deformation_lr_final*self.spatial_lr_scale,
    ...
    # def update_learning_rate_jih(self, iteration, stage):
    ...
    def load_model(self, path):
        ...
        print("path",path)
        self._scene_flow = torch.load(os.path.join(path,"scene_flow.pth"))
    ...
    def save_deformation(self, path):
        ...
        torch.save(self._scene_flow,os.path.join(path, "scene_flow.pth"))
    ...
    def prune_points(self, mask):
        ...
        self._scene_flow = self._scene_flow[valid_points_mask]
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, new_deformation_table, new_sceneflow):
        d = {"xyz": new_xyz,
        ...
        self.denom = torch.zeros((self.get_xyz.shape[0], 1), device="cuda")
        self.max_radii2D = torch.zeros((self.get_xyz.shape[0]), device="cuda")
        self._scene_flow = torch.cat([self._scene_flow, new_sceneflow])     # 일단 shape은 xyz랑 같아짐
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        new_sceneflow = self.get_flow[selected_pts_mask].repeat(N, 1)

        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_opacity, new_scaling, new_rotation, new_deformation_table, new_sceneflow)
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent, density_threshold=20, displacement_scale=20, model_path=None, iteration=None, stage=None):
        ...
        new_sceneflow = self._scene_flow[selected_pts_mask]

        # if opt.add_point:
        # selected_xyz, grow_xyz = self.add_point_by_mask(selected_pts_mask_grow.to(self.get_xyz.device), self.displacement_scale)
        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, new_deformation_table, new_sceneflow)
