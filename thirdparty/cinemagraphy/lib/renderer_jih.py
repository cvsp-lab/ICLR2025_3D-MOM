...
class ImgRenderer():
    ...
    def render_pcd(self, middle_index , pts, rgbs, feats, mask, side_ids, R=None, t=None, scale_weight=1, time=0, t_step=None, path_type=None, t_all = None):
        ...
        # extrinsic = torch.cat([R, t.view(3, 1)], dim=1)
        ...
        meta = {}
        for j in range(num_sides):      #num_sides==2
            ...
            global_out_j = renderer(all_pcd_j)      # all_pcd_j: feature point cloud , feat = feat + RGB
            all_colored_pcd_j = Pointclouds(points=[pts_[mask_side]], features=[rgbs[mask_side]])     
            direct_rgb_out_j = renderer(all_colored_pcd_j)
            global_out_list.append(global_out_j)            # global_out_j.shape torch.Size([1, 720, 1280, 37])
            direct_color_out_list.append(direct_rgb_out_j)

        # len(global_out_list) 2
        print("global_out_list[0][..., [3]]",global_out_list[0][..., [3]])
        print("global_out_list[-1][..., [3]]",global_out_list[-1][..., [3]])

        w1, w2 = self.compute_weight_for_two_frame_blending(time,
                                                            global_out_list[0][..., [-1]],
                                                            global_out_list[-1][..., [-1]],
                                                            global_out_list[0][..., [3]],       # alpha map 1  # global_out_list[0][..., [3]].shape torch.Size([1, 720, 1280, 1]) 
                                                            global_out_list[-1][..., [3]]       # alpha map 2    
                                                            )
        
        direct_rgb_out = w1 * direct_color_out_list[0] + w2 * direct_color_out_list[-1]
        
        color_1= direct_color_out_list[0]
        color_2= direct_color_out_list[-1] 
        
        direct_render_image = color_1[..., :3].permute(0, 3, 1, 2)       
        direct_color_2 = color_2[..., :3].permute(0, 3, 1, 2)
        ...
        return pred_rgb, direct_rgb, meta, direct_render_image, self.intrinsic.squeeze()[:3, :3]
    ...
    def render_depth_from_mdi(self, depth_layers, alpha_layers):
        ...
        inpainting한 depth -> make it 4D에서는 새롭게 depth map을 생성 
        
        우리도 inpainting한 이미지에 누적하면서 depth를 새롭게 뽑고
        이후에 min-max normalization 진행 
        지금 상황에서 일단 inpainting한 depth를 normalization까지는 한 depth
        그럼 weight를 알파 값으로 하는 것이 아닌 d1,d1..의 클러스터링 기준 값을 해보자!
        '''
        ...
        # '''
        depth_layer_for_pc = weights * depth_layers
        depth_layer_for_pc = torch.clamp(depth_layer_for_pc, min=1.)      # sum 하지 말고 레이어별 depth로 유지
        return depth_map, layer_id_map, depth_layer_for_pc
    ...
    def compute_scene_flow_for_motion(self, middle_index, coord, pose,
                                      rgb,
                                      rgba_layers,
    ...
    def compute_flow_and_inpaint(self):
        with torch.no_grad():
            ...
            outputs_motion = self.scene_flow_estimator.forward_flow(image, mask, hint)      # motion estimation model (motion_model.py)
            flow = outputs_motion["PredMotion"] * mask  # mask 영역에 대해서만 segmentation motion 적용
            for _ in range(7):
                flow_ = box_blur(flow, (15, 15), border_type='constant')
            flow = flow_ * mask
            flow_scale = [self.w / flow.shape[3], self.h / flow.shape[2]]
            flow = flow * torch.FloatTensor(flow_scale).to(flow.device).view(1, 2, 1, 1)    # flow에 대해 
        ...
        rgba_layers_src, depth_layers_src, mask_layers_src = \
            self.inpainter.sequential_inpainting(self.src_img, self.src_depth, depth_bins_src)      #rgba_layers_src.shape=([4, 1, 4, 512, 896]), depth_layers_src.shape=([4, 1, 1, 512, 896])
        featmaps_src = self.feature_extraction(rgba_layers_src, mask_layers_src, depth_layers_src)  #featmaps_src.shape = ([4, 32, 128, 224])
        depth_layers_src = self.apply_scale_shift(depth_layers_src, self.scale_shift[0], self.scale_shift[1])   
        depth_layers_src_ = depth_layers_src.clone().cpu().numpy()
        # depth_layers_src.shape = torch.Size([3, 1, 1, 720, 1280])
        processed_depth_src, layer_id_map_src, depth_layer_for_pc = self.render_depth_from_mdi(depth_layers_src, rgba_layers_src[:, :, -1:])    # processed_depth_src 인패인팅 후 추정한 depth layer
        h, w = self.src_img.shape[-2:]
        coord = get_coord_grids_pt(h, w, device=self.device).float()[None]                              # get_coord_grids_pt: 이미지의 높이와 너비를 통해 픽셀 좌표 그리드를 생성
        pts_src = unproject_pts_pt(self.intrinsic, coord.reshape(-1, 2), processed_depth_src.flatten()) # unproject_pts_pt: 그리드와 해당 좌표에 대한 깊이 정보를 사용하여 3D 포인트로 변환 , pts_src가 포인트 클라우드 맞음 (color값이 있는지 알수)
        """Extract Point Cloud data"""
        image_list = []
        depth_list = []
        
        # print("rgba_layers_src.shape",depth_layer_for_pc.shape)
        # print("depth_layer_for_pc.shape",depth_layer_for_pc.shape)
        # print("processed_depth_src.shape",processed_depth_src.shape)
        
        for i in range (rgba_layers_src.shape[0]): 
            image_list.append(rgba_layers_src[i][0][:3])            # image_list[0].shape torch.Size([3, 720, 1280])
            depth_list.append(depth_layer_for_pc[i][0][0])          # depth_list[0].shape torch.Size([720, 1280])

        # print("len(image_list)",len(image_list))
        # print("len(depth_list)",len(depth_list))
        # print("image_list[0].shape",image_list[0].shape)
        # print("depth_list[0].shape",depth_list[0].shape)
        
        return coord, flow, pts_src, featmaps_src, rgba_layers_src, depth_layers_src, mask_layers_src, processed_depth_src, self.src_img, image_list, depth_list
    
    
    def compute_flow_and_inpaint_jih(self):
        with torch.no_grad():
            image = self.motion_rgbs
            mask = self.mask[None]
            hint = self.hints[None]
        ...
        depth_bins_src = get_depth_bins_jih(depth=self.src_depth)
        # depth map,rgba inpainting
        rgba_layers_src, depth_layers_src, mask_layers_src = \
            self.inpainter.sequential_inpainting(self.src_img, self.src_depth, depth_bins_src)      #rgba_layers_src.shape=([4, 1, 4, 512, 896]), depth_layers_src.shape=([4, 1, 1, 512, 896])

        featmaps_src = self.feature_extraction(rgba_layers_src, mask_layers_src, depth_layers_src)
        depth_layers_src = self.apply_scale_shift(depth_layers_src, self.scale_shift[0], self.scale_shift[1])                                   #depth map scailing    
        
        processed_depth_src, layer_id_map_src, depth_layer_for_pc = self.render_depth_from_mdi(depth_layers_src, rgba_layers_src[:, :, -1:])    
        ...
        for i in range (rgba_layers_src.shape[0]): 
            ...
            depth_list.append(depth_layer_for_pc[i][0][0])          # depth_list[0].shape torch.Size([720, 1280])
        # '''
        return coord, flow, pts_src, featmaps_src, rgba_layers_src, depth_layers_src, mask_layers_src, image_list, depth_list
