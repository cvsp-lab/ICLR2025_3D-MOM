...
class ImgRenderer():
    ...
    def render_pcd(self, pts, rgbs, feats, mask, side_ids, R=None, t=None, time=0, t_step=None, path_type=None):
        ...
        return pred_rgb, direct_rgb, meta, R_out, t_out
    ...
    def compute_flow_and_inpaint(self):
        ...
        # depth_bins_src =  [tensor(1.0020, device='cuda:0'), 1.5662867537441163, 4.427143243176135, tensor(2340.5356, device='cuda:0')]  -> 클러스터링 기준
        # len(depth_bins_src) 4
        prompt = self.args.prompt
        input_dir = self.args.input_dir

        # source image, depth, depth_binse(depth 클러스터링 결과 )
        rgba_layers_src, depth_layers_src, mask_layers_src = \
            self.inpainter.sequential_inpainting(self.src_img, self.src_depth, depth_bins_src, prompt,input_dir)
